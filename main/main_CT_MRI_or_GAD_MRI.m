%% Code for CT/MRI and GAD/MRI fusion.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc;clear all;close all;

ImgA = imread('E:\sourceimage\CT+T2\CT\1.jpg'); %image path of CT or GAD
ImgB = imread('E:\sourceimage\CT+T2\T2\1.jpg'); %image path of MRI
if size(ImgA,3)>1
    A=im2double(ImgA);
...
uA = FRAGL(A,M,N);
uB = FRAGL(B,M,N); 
segA = double((0.8<uA)&(uA<1));
segB = double((0.8<uB)&(uB<1));
sA=A.*segA;
sB=B.*segB;
edgeMap_imA=edgedetection(sA,M,N,p);
edgeMap_imB=edgedetection(sB,M,N,p);
